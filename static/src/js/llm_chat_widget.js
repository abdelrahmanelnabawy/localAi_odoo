...
export class LLMChatWidget extends Component {
    ...
    async onFileChange(ev) {
        ...
        if (file) {
            ...
            reader.onload = () => {
                // Store file as Base64 string in state
    ...
    clearFile() {
        ...
        if (this.fileInputRef.el) {
            this.fileInputRef.el.value = ""; // Crucial: allows re-selecting the same file
        }
    }
    async sendMessage() {
        const messageText = this.state.inputMessage.trim();
        const attachedFile = this.state.fileData; // Base64
        const attachedFileName = this.state.fileName;
    
        // Don't send if there is neither text nor a file
        if (!messageText && !attachedFile) return;
        // Create the message object for the UI
        const newMessage = {
            role: "user",
            content: messageText,
            file: attachedFile,      // Store the file with THIS specific message
        ...
        // CLEAR the inputs and the "pending" file immediately so the next message is fresh
        ...
        try {
            const result = await rpc("/llm/chat", {
                ...
                message: messageText,
                file: attachedFile,      // Send the file data to Python
                filename: attachedFileName,
            });
